"""
Matlama Scalper Engine - ML Trainer
Trains a Random Forest classifier on logged trade history
(scalper_trades.csv, written by MatlamaScalper.mq5 into the MT5 sandbox
Files folder).

CSV Schema:
ticket, symbol, type, open_time, close_time, open_price, close_price,
volume, profit, swap, commission, duration_min, ema_diff_pips,
momentum_pips, rsi
...
Saves model + scaler to C:\\Matlama\\model\\ as scalper_model.pkl /
scalper_scaler.pkl, which the orchestrator loads for the "SCALPER" strategy.

Run manually: python scalper_engine.py
Task Scheduler: Matlama_ScalperEngine (daily, e.g. 5:30 AM)
"""
...
STATE_FILE = os.path.join(MODEL_DIR, "scalper_train_state.json")

MT5_SANDBOX_FILES = (
    r"C:\Users\Administrator\AppData\Roaming\MetaQuotes\Terminal"
    r"\5A08E185CE336B177334803F286A1E5F\MQL5\Files"
)
TRADES_CSV = os.path.join(MT5_SANDBOX_FILES, "scalper_trades.csv")

MIN_TRADES_TO_TRAIN = 50
RETRAIN_INTERVAL_HOURS = 24

FEATURE_COLS = ["ema_diff_pips", "momentum_pips", "rsi"]

os.makedirs(MODEL_DIR, exist_ok=True)
os.makedirs(LOG_DIR, exist_ok=True)

logging.basicConfig(
    filename=os.path.join(LOG_DIR, "scalper_engine.log"),
    level=logging.INFO,
...
# ---------------------------------------------------------------------------
def main():
    logger.info("=== scalper_engine.py started ===")
    ...
    joblib.dump(model, os.path.join(MODEL_DIR, "scalper_model.pkl"))
    joblib.dump(scaler, os.path.join(MODEL_DIR, "scalper_scaler.pkl"))
    ...
    logger.info("=== scalper_engine.py finished ===")
