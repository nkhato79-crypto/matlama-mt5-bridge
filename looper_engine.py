"""
Matlama Looper Engine - ML Trainer
Trains a Random Forest classifier on logged trade history
(looper_trades.csv, written by MatlamaLooper.mq5 into the MT5 sandbox
Files folder).

CSV Schema:
ticket, symbol, type, open_time, close_time, open_price, close_price,
volume, profit, swap, commission, duration_min, band_position,
range_width_pips, rsi, volume_ratio

Result is derived from profit > 0 (win=1, loss=0).

duration_min is logged for record-keeping but excluded from FEATURE_COLS
-- it's only known after a trade closes.

Retrains on a 24-hour cycle, only if at least 50 closed trades are available.
Saves model + scaler to C:\\Matlama\\model\\ as looper_model.pkl /
looper_scaler.pkl, which the orchestrator loads for the "LOOPER" strategy.

Run manually: python looper_engine.py
Task Scheduler: Matlama_LooperEngine (daily, e.g. 6:00 AM)
"""
...
STATE_FILE = os.path.join(MODEL_DIR, "looper_train_state.json")

MT5_SANDBOX_FILES = (
    r"C:\Users\Administrator\AppData\Roaming\MetaQuotes\Terminal"
    r"\5A08E185CE336B177334803F286A1E5F\MQL5\Files"
)
TRADES_CSV = os.path.join(MT5_SANDBOX_FILES, "looper_trades.csv")

MIN_TRADES_TO_TRAIN = 50
RETRAIN_INTERVAL_HOURS = 24

FEATURE_COLS = ["band_position", "range_width_pips", "rsi", "volume_ratio"]

os.makedirs(MODEL_DIR, exist_ok=True)
os.makedirs(LOG_DIR, exist_ok=True)

logging.basicConfig(
    filename=os.path.join(LOG_DIR, "looper_engine.log"),
    level=logging.INFO,
    format="%(asctime)s [%(levelname)s] %(message)s",
)
logger = logging.getLogger("looper_engine")
...
def load_trades():
    ...
    if "profit" not in df.columns:
        logger.error("No 'profit' column found in CSV")
        return None
...
# ---------------------------------------------------------------------------
# Training -- picks best of RF vs GBM (same approach as QuantEngine)
# ---------------------------------------------------------------------------
def train_model(df):
    available_features = [c for c in FEATURE_COLS if c in df.columns]
    ...
    if gbm_acc > rf_acc + 0.02:
        model, acc, model_type = gbm, gbm_acc, "GBM"
    else:
        model, acc, model_type = rf, rf_acc, "RF"

    logger.info("Selected %s (acc=%.3f) over %s (acc=%.3f)",
                model_type, acc, "RF" if model_type == "GBM" else "GBM",
                rf_acc if model_type == "GBM" else gbm_acc)

    report = classification_report(y_test, model.predict(X_test_s), zero_division=0)
...
# ---------------------------------------------------------------------------
def main():
    logger.info("=== looper_engine.py started ===")
    ...
    joblib.dump(model, os.path.join(MODEL_DIR, "looper_model.pkl"))
    joblib.dump(scaler, os.path.join(MODEL_DIR, "looper_scaler.pkl"))
    ...
    logger.info("=== looper_engine.py finished ===")
